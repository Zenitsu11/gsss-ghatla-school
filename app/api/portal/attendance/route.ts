...
export async function GET(){
  ...
  const q=s.role==="student"?db.from("attendance").select("*").eq("student_id",s.id).order("attendance_date",{ascending:false}).limit(60):db.from("attendance").select("*").order("attendance_date",{ascending:false}).limit(100);
