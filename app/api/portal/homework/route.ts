...
export async function GET(){
  const s=readPortalSession((await cookies()).get("gsss_portal")?.value);
  if(!s)return NextResponse.json({error:"Unauthorized"},{status:401});
  const db=supabaseAdmin();
  if(!db)return NextResponse.json({error:"Supabase not configured"},{status:500});
  const q=s.role==="student"?db.from("homework").select("*").eq("class_name",(await db.from("school_users").select("class_name").eq("id",s.id).single()).data?.class_name||"").order("due_date",{ascending:true}):db.from("homework").select("*").order("created_at",{ascending:false});
